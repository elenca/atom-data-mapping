...
def set_kartei(value, kartei):
    if value is not np.nan:
        return "Kartei " + kartei + ": " + str(value)
    else:
        value
...
### data preparation ###
    ...
    data['eventStartDates'] = data[['Zeitraum von', 'Datierung']].apply(lambda x: x['Datierung'] if x['Datierung'] is not np.nan else x['Zeitraum von'], axis=1)
    data['eventEndDates'] = data[['Zeitraum bis', 'Datierung']].apply(lambda x: x['Datierung'] if x['Datierung'] is not np.nan else x['Zeitraum bis'], axis=1)
    ...
    data['nameAccessPoints'].loc[data['Preis'].notnull() == True] = data['nameAccessPoints'] + "|" + data['Preis'] + " (Ehrung)"
    ...
    data['eventStartDates'] = data['eventStartDates'].astype(str) + "|" + data['eventStartDates'].astype(str)
    data['eventEndDates'] = data['eventEndDates'].astype(str) + "|" + data['eventEndDates'].astype(str)
    ...
    """Create "findingAids"
    """
    # create column for "findingsAids" 
    # and add values from "Kartei Heller" and "Kartei Weidmann"
    data['Kartei Heller'] = data['Kartei Heller'].apply(set_kartei, kartei=('Heller'))
    data['Kartei Weidmann'] = data['Kartei Weidmann'].apply(set_kartei, kartei=('Weidmann'))
    data['findingAids'] = data['Kartei Weidmann'] + ", " + data['Kartei Heller']

    """Create "extentAndMedium"
    """
...
### levels of description ###
    ...
    mydata.rename(columns={
        ...
        #'Original Dateiname / Signatur': 'identifier', #not working
        #'Original Dateiname / Signatur': 'Teilserie', #TODO: check this field
        'Alternativer Titel': 'alternateTitle',
